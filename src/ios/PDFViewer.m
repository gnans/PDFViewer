...
- (void)viewPDF:(CDVInvokedUrlCommand*)command
{
    ...
    if (@available(iOS 11.0, *)) {
        @try {
            ...
            self.pdfView = [[PDFView alloc] initWithFrame:CGRectMake(0, 70, self.webView.bounds.size.width, self.webView.bounds.size.height)];
            ...
            self.toolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 20,        self.webView.bounds.size.width, 90)];
            self.toolbar.barTintColor = [UIColor colorWithRed:243/255.0 green:241/255.0 blue:238/255.0 alpha:1.0];
            self.toolbar.backgroundColor = [UIColor colorWithRed:243/255.0 green:241/255.0 blue:238/255.0 alpha:1.0];
            [self.toolbar sizeToFit];
            [self.webView addSubview:self.toolbar];
            [self.webView bringSubviewToFront: self.toolbar];
              
            // toolbar title
            self.label = [[UILabel alloc] initWithFrame:CGRectMake((self.webView.bounds.size.width/2) - 150, 30, 50, 40)];
            ...
            self.button.frame = CGRectMake(20, 20, 50, 40.0);
            [self.webView addSubview:self.button];
            [self.webView bringSubviewToFront:self.button];
            
            self.webView.autoresizingMask = UIViewAutoresizingFlexibleRightMargin |
